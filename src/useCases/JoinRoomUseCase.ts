...
export class JoinRoomUseCase {
  constructor(private wordsRepository: RoomWordsRepository, private roomsRepository: RoomsRepository) {}
  ...
  async execute({ rooms, roomId, user }: JoinRoomUseCaseParams) {
    ...
    if (!room) {
      ...
      if(room.players.some(roomPlayer => roomPlayer.email === player.email)) {
